...
type StatOrderBestSoldProps = {
  idRestaurant: number;
};

export const StatOrderBestSold = ({ idRestaurant }: StatOrderBestSoldProps) => {
  const [dateMin, setDateMin] = useState<string>("");

  return (
    <div className="col-start-2 row-start-1 py-4 px-16 flex flex-col gap-2">
      <div className="w-full flex justify-between items-center">
        <span className="text-lg">Top 5 plats achetés</span>
        <Select
          onValueChange={(e) =>
      ...
      <div>
        {dateMin !== "" && idRestaurant !== 0 ? (
          <StatTableBestSold dateMin={dateMin} idRestaurant={idRestaurant} />
        ) : (
