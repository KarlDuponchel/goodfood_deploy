...
export const ProductBasket: FunctionComponent<ProductBasketProps> = ({
  ...
  const restaurant = useFetchRestaurantById(
    product.data && product.data?.id_restaurant
      ? product.data.id_restaurant
      : -1
  );
        ...
        <div className="flex gap-3">
          <div className="mb-2">
            <a href={`/products/${productSent.idContent}`}>
              <Image
                ...
                className="w-16 h-16 bg-center rounded-full object-cover"
                width={45}
                height={45}
              />
            </a>
          ...
          <div className="flex flex-col">
            <span className="text-lg font-extrabold">
              {restaurant.data?.name} - {productSent.contentName} (
              {product.data?.price}€)
            </span>
            {adresseClient ? (
        ...
        <div className="relative flex flex-col items-end gap-3">
          <div className="flex gap-1">
            <BaseNbSelect
              ...
              onChange={() =>
                status === 1
                  ? updateQuandityBasket(
                      product.data && product.data.ID ? product.data.ID : -1
                    )
                  : updateNbProducts(productSent.idContent)
              }
              defaultValue={productSent.quantity}
            />
            <TrashIcon
              className="absolute cursor-pointer w-6 text-red-500 -right-7 top-10"
              onClick={() =>
                status === 1
                  ? deleteFromBasket(
                      product.data && product.data.ID ? product.data.ID : -1
                    )
                  : deleteFromCart(productSent.idContent)
              }
            />
          </div>
